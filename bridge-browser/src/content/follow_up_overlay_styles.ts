...
export const FOLLOW_UP_OVERLAY_STYLE_TEXT = `
  :host { position: fixed; left: 20px; bottom: 20px; z-index: ${FOLLOW_UP_OVERLAY_Z_INDEX};
    width: min(360px, calc(100vw - 32px)); max-height: calc(100vh - 32px); color-scheme: dark; }
  * { box-sizing: border-box; }
  button, textarea { font: inherit; }
  .panel { width: 100%; max-height: inherit; display: flex; overflow: hidden; flex-direction: column;
  ...
  .item { display: flex; align-items: flex-start; gap: 8px; padding: 8px 10px 8px 12px;
  ...
  .remove { width: 22px; height: 22px; flex: 0 0 auto; padding: 0; border: 0; border-radius: 5px;
    color: #aeb5c2; background: transparent; cursor: pointer; }
