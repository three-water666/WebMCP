...
| `attach_file` | Pastes a PNG, JPEG, WebP, GIF, or PDF file of up to 20 MB from the workspace into the current AI conversation as a real attachment for a capable site and model to inspect. If the page does not acknowledge the paste, an attachment delivery error is returned under the original `request_id`. |
