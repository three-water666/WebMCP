...
5. **优先使用专用文件工具**：当 {{PRODUCT_NAME}} Available Tools 中提供专用文件工具时，查找工作区文件优先用 `search_files`，搜索代码或文本内容优先用 `search_code`，读取文本内容或行范围优先用 `read_file`，查看图片或 PDF 优先用 `attach_file`，修改已有文件优先用 `edit_file`。不要为了查看文件而用 `execute_command` 执行 `grep`、`rg`、`find`、`cat`、`sed`、`awk`、`nl` 等 shell 命令；`execute_command` 应主要用于构建、测试、包管理器、git 命令和项目脚本。
